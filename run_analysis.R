...
#To obtain the final tidy data set 
...
moltenData <- melt(completeDataSet, id =c("subject", "activity"), measure.vars = 4:69, variable.name = "measurement", value.name = "value")

tidyDataSet <- dcast(moltenData, subject+activity ~ measurement, mean)

#outputting csv file
write.table(tidyDataSet, "myTidyDataSet2.txt", row.names = FALSE, quote = FALSE)
write.xlsx(tidyDataSet, "myTidyDataSet2.xlsx", row.names = FALSE)
